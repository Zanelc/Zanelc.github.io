...
**注：** **st_name** 代表的是目标文件中 **字符串表** 中的一个索引值
...
例： **func1** 和 **main** 函数都是定义在simplesection.c里面的，他们所在的位置都为代码段，所以 **Ndx** 为1，它们都是函数，类型为 **STT_FUNC** ，都是全局可见的，所以是 **STB_GLOBAL** ，**Size** 表示函数指令所占的字节数；**Value** 表示函数相对于代码段起始位置的偏移。
